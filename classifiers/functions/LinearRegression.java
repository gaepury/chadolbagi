...
import com.example.g.cardet.core.Matrix;
...
public class LinearRegression extends AbstractClassifier implements OptionHandler,
  ...
  private boolean deselectColinearAttributes(boolean [] selectedAttributes,
    ...
    if (maxAttr >= 0) {
      ...
      if (b_Debug) {
	Log.i("clustering", "Deselected colinear attribute:" + (maxAttr + 1)
  ...
  private void findBestModel() throws Exception {
    ...
    if (b_Debug) {
      Log.i("clustering", (new Instances(m_TransformedData, 0)).toString());
    }
    ...
    if (b_Debug) {
      Log.i("clustering", "Initial Akaike value: " + akaike);
    }
	...
	for (int i = 0; i < m_SelectedAttributes.length; i++) {
	  if (currentSelected[i]) {
	    ...
	    if (b_Debug) {
	      Log.i("clustering", "(akaike: " + currentAkaike);
	    }
  ...
  throws Exception {
    ...
    if (b_Debug) {
      Log.i("clustering", "doRegression(");
      for (int i = 0; i < selectedAttributes.length; i++) {
	Log.i("clustering", " " + selectedAttributes[i]);
      }
      Log.i("clustering", " )");
    }
    int numAttributes = 0;
