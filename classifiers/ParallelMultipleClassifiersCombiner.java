...
public abstract class ParallelMultipleClassifiersCombiner extends
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    if (m_numExecutionSlots > 1) {
      if (m_Debug) {
        Log.i("clustering", "Starting executor pool with " + m_numExecutionSlots
            + " slots...");
      }
      startExecutorPool();
    }
      ...
      if (m_numExecutionSlots > 1) {
        ...
        Runnable newTask = new Runnable() {
          public void run() {
            try {
              if (m_Debug) {
                Log.i("clustering", "Training classifier (" + (iteration +1) + ")");
              }
              currentClassifier.buildClassifier(data);
              if (m_Debug) {
                Log.i("clustering", "Finished classifier (" + (iteration +1) + ")");
              }
