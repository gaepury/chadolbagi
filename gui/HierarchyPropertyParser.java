...
package com.example.g.cardet.gui;
...
public class HierarchyPropertyParser
	...
	try{
	    ...
	    Log.i("clustering", "seperator: "+hpp.getSeperator());
	    Log.i("clustering", "depth: "+hpp.depth());
	    Log.i("clustering", "The tree:\n\n"+hpp.showTree());
	    hpp.goToRoot();
	    Log.i("clustering", "goto: "+hpp.goTo("node1.node1_2.node1_2_1")
			       +": "+hpp.getValue()+" | "+hpp.fullValue()+
			       " leaf? "+ hpp.isLeafReached());
	    Log.i("clustering", "go down(wrong): "+hpp.goDown("node1"));
	    Log.i("clustering", "Stay still? "+hpp.getValue());
	    Log.i("clustering", "go to child: "+hpp.goToChild("node1_2_1_1")
			       +": "+hpp.getValue()+" | "+hpp.fullValue()+
			       " leaf? "+ hpp.isLeafReached()
			       +" root? "+ hpp.isRootReached());
	    Log.i("clustering", "parent: "+hpp.parentValue());
	    Log.i("clustering", "level: "+hpp.getLevel());
	    Log.i("clustering", "context: "+hpp.context());
	    hpp.goToRoot();
	    Log.i("clustering", "After gotoRoot. leaf? "+ hpp.isLeafReached()
			       +" root? "+ hpp.isRootReached());
	    Log.i("clustering", "Go down(correct): "+
			       ...
			       +" root? "+ hpp.isRootReached());
	    hpp.goToParent();
	    Log.i("clustering", "value: "+hpp.getValue()+" | "+hpp.fullValue());
	    Log.i("clustering", "level: "+hpp.getLevel());
	    
	    String[] chd = hpp.childrenValues();
	    for(int i=0; i < chd.length; i++){
		Log.i("clustering", "children "+i+": "+chd[i]);
		hpp.goDown(chd[i]);
		Log.i("clustering", "real value: "+hpp.getValue()+" | "+
				   hpp.fullValue()+
				   "(level: "+hpp.getLevel()+")");
		hpp.goToParent();
	    }
	    
	    Log.i("clustering", "Another way to go to root:"+hpp.goTo("node1")
			       +": "+hpp.getValue()+" | "+hpp.fullValue()); 
   	}catch(Exception e){
