...
package com.example.g.cardet.experiment;
...
public class PairedTTester 
  ...
  protected class Resultset
	...
	// compact the string
        tempResult = Utils.removeSubstring(tempResult, "com.example.g.cardet.classifiers.");
        tempResult = Utils.removeSubstring(tempResult, "com.example.g.cardet.filters.");
        tempResult = Utils.removeSubstring(tempResult, "com.example.g.cardet.attributeSelection.");
	result += tempResult;
      }
      return result.trim();
    }
  ...
  protected String templateString(Instance template) {
    ...
    if (result.startsWith("com.example.g.cardet.classifiers.")) {
      result = result.substring("com.example.g.cardet.classifiers.".length());
    }
    return result.trim();
  }
  ...
  public static void main(String args[]) {
	...
	throw new Exception(
      ...
      Log.i("clustering", tt.header(compareCol));
      if (rankingOnly) {
	Log.i("clustering", tt.multiResultsetRanking(compareCol));
      } else if (summaryOnly) {
	Log.i("clustering", tt.multiResultsetSummary(compareCol));
      } else {
	Log.i("clustering", tt.resultsetKey());
	if (baseColStr.length() == 0) {
	  for (int i = 0; i < tt.getNumResultsets(); i++) {
            if (!tt.displayResultset(i))
              continue;
	    Log.i("clustering", tt.multiResultsetFull(i, compareCol));
	  }
	} else {
	  int baseCol = Integer.parseInt(baseColStr) - 1;
	  Log.i("clustering", tt.multiResultsetFull(baseCol, compareCol));
	}
      }
    } catch(Exception e) {
      e.printStackTrace();
