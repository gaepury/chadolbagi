...
package com.example.g.cardet.core.neighboursearch.kdtrees;
...
public class KMeansInpiredMethod
  ...
  public void splitNode(KDTreeNode node, int numNodesCreated,
    ...
    if (rightStart == node.m_Start || rightStart > node.m_End) {
      Log.i("clustering", "node.m_Start: " + node.m_Start + " node.m_End: "
  ...
  private static void checkSort(Instances insts, int[] indices, int attidx, 
    ...
    for(int i=start+1; i<=end; i++) {
      if( insts.instance(indices[i-1]).value(attidx) > 
        ...
        Log.i("clustering", "value[i-1]: "+insts.instance(indices[i-1]).value(attidx));
        Log.i("clustering", "value[i]: "+insts.instance(indices[i]).value(attidx));
        Log.i("clustering", "indices[i-1]: "+indices[i-1]);
        Log.i("clustering", "indices[i]: "+indices[i]);
        Log.i("clustering", "i: "+i);
        if(insts.instance(indices[i-1]).value(attidx) > insts.instance(indices[i]).value(attidx))
          Log.i("clustering", "value[i-1] > value[i]");
