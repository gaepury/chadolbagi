...
public class Queue
  ...
  public static void main(String [] argv) {
	...
	queue.push(argv[i]);
      ...
      Log.i("clustering", "After pushing command line arguments");
      Log.i("clustering", queue.toString());
      while (!queue.empty()) {
	Log.i("clustering", "Pop: " + queue.pop().toString());
      }
      // try one more pop, to make sure we get an exception
      try 
	{
	  queue.pop();
	  Log.i("clustering", "ERROR: pop did not throw exception!");
	}
      catch (RuntimeException ex)
        {
	  Log.i("clustering", "Pop on empty queue correctly gave exception.");
	}
    } catch (Exception ex) {
