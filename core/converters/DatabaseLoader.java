      ...
      if(m_checkForTable) {
        ...
        while (!rightChoice){
            try{
                ...
                choice++;
                if(choice == 3){
                    Log.i("clustering", "Incremental loading not supported for that DBMS. Pseudoincremental mode is used if you use incremental loading.\nAll rows are loaded into memory once and retrieved incrementally from memory instead of from the database.");
...
//      String attribName = md.getColumnName(i + 1);
	...
	printException(ex);
        ...
        if(m_query.equals("Select * from Results0")){
            ...
            Log.i("clustering", text.toString());
        }
    }
  ...
  private void printException(Exception ex){
      ...
      Log.i("clustering", "\n--- Exception caught ---\n");
	while (ex != null) {
		Log.i("clustering", "Message:   "
                                   + ex.getMessage ());
                if(ex instanceof SQLException){
                    Log.i("clustering", "SQLState:  "
                                   + ((SQLException)ex).getSQLState ());
                    Log.i("clustering", "ErrorCode: "
  ...
  public String getRevision() {
    return RevisionUtils.extract("$Revision: 7039 $");
  }
  ...
  public static void main(String [] options) {
	...
	atf = new DatabaseLoader();
        ...
        if(!atf.m_inc)
            Log.i("clustering", atf.getDataSet().toString());
        else{
            Instances structure = atf.getStructure();
            Log.i("clustering", structure.toString());
            Instance temp;
            do {
            temp = atf.getNextInstance(structure);
            if (temp != null) {
                Log.i("clustering", temp.toString());
            }
            } while (temp != null);
        }
      } catch (Exception e) {
