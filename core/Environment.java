...
public class Environment implements RevisionHandler {
  ...
  public Environment() {
    ...
    m_envVars.put("com.example.g.cardet.version", Version.VERSION);
  }
  ...
  public static void main(String[] args) {
    ...
    if (args.length == 0) {
      System.err.println("Usage: java com.example.g.cardet.core.Environment <string> <string> ...");
    } else {
      try {
        for (int i = 0; i < args.length; i++) {
          String newS = t.substitute(args[i]);
          Log.i("clustering", "Original string:\n" + args[i] +"\n\nNew string:\n" + newS);
        }
      } catch (Exception ex) {
        ex.printStackTrace();
      }
