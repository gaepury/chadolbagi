...
import com.example.g.cardet.filters.SimpleStreamFilter;
...
public class NumericCleaner
  ...
  protected Instance process(Instance instance) throws Exception {
	...
	if (getDebug())
	  Log.i("clustering", "Too small: " + result.value(i) + " -> " + m_MinDefault);
	result.setValue(i, m_MinDefault);
      }
      // too big?
      else if (result.value(i) > m_MaxThreshold) {
	if (getDebug())
	  Log.i("clustering", "Too big: " + result.value(i) + " -> " + m_MaxDefault);
	...
	if (getDebug())
	  Log.i("clustering", "Too close: " + result.value(i) + " -> " + m_CloseToDefault);
