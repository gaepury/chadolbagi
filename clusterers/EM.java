...
package com.example.g.cardet.clusterers;
...
import com.example.g.cardet.estimators.DiscreteEstimator;
...
public class EM
    ...
    private void EM_Report (Instances inst) {
        ...
        for (j = 0; j < m_num_clusters; j++) {
            for (i = 0; i < m_num_attribs; i++) {
                Log.i("clustering", "Clust: " + j + " att: " + i + "\n");

                if (m_theInstances.attribute(i).isNominal()) {
                    if (m_model[j][i] != null) {
                        Log.i("clustering", m_model[j][i].toString());
                    }
                }
                else {
                    Log.i("clustering", "Normal Distribution. Mean = "
        ...
        for (l = 0; l < inst.numInstances(); l++) {
            ...
            Log.i("clustering", "Inst " + Utils.doubleToString((double)l, 5, 0)
                    + " Class " + m + "\t");
            for (j = 0; j < m_num_clusters; j++) {
                Log.i("clustering", Utils.doubleToString(m_weights[l][j], 7, 5) + "  ");
            }
        }
    }
    ...
    private void CVClusters ()
        ...
        CLUSTER_SEARCH: while (CVincreased) {
            ...
            for (i = 0; i < numFolds; i++) {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    Log.i("clustering", "Restarting after CV training failure ("+num_clusters+" clusters");
                    seed++;
                    restartCount++;
                    ok = false;
                    if (restartCount > 5) {
                ...
                try {
                    ...
                    //          ex.printStackTrace();
                    ex.printStackTrace();
                    Log.i("clustering", "Restarting after CV testing failure ("+num_clusters+" clusters");
                ...
                if (m_verbose) {
                    Log.i("clustering", "# clust: " + num_clusters + " Fold: " + i
                            + " Loglikely: " + tll);
                }
                templl += tll;
            }
            ...
            if (ok) {
                ...
                //if (m_verbose)
                    Log.i("clustering", System.currentTimeMillis() + "\n==================================="
                            + "==============\n# clust: "
                            + num_clusters
                            + "   Mean Loglikely: "
                            + templl
                            + "\n# CVLogLikely : " + CVLogLikely
                            + "\n================================"
                            + "=================");
        ...
        if (m_verbose)
            Log.i("clustering", System.currentTimeMillis() + ": Number of clusters: " + (num_clusters - 1));
    ...
    public void buildClusterer (Instances data)
        ...
        data = com.example.g.cardet.filters.Filter.useFilter(instances, m_replaceMissing);
        instances = null;
    ...
    private void doEM ()
        ...
        //if (m_verbose)
            Log.i("clustering", "Number of instances: "
                    + m_num_instances
                    + "\nNumber of atts: "
                    + m_num_attribs
                    + "\n");
        ...
        while (!ok) {
            try {
                for (i = 0; i < m_max_iterations; i++) {
                    ...
                    if (report) {
                        Log.i("clustering", "Loglikely: " + llk);
                    }
